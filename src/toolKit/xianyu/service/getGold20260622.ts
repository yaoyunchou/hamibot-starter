...
export function mainPopupFn(title: string, callback: () => void, task: any) {
  ...
  while (scrollAttempts <= TASK_SCROLL_MAX) {
    ...
    if (!titleEl) {
      if (isTaskPopupOpen()) {
        // 弹框在但找不到该任务标题，说明服务端已将此任务标记完成（行消失）
        setRunInfo(`mainPopupFn[${title}]: 标题不在弹框列表，视为已完成`);
        task.hasRun = true;
        return;
      }
    ...
    // 先标记状态，防止 backMainPage → findPage → coinExchange 重入时该任务被重复执行
    ...
    if (!assertOnGoldCoinPage(`mainPopupFn[${title}] 完成后`, 1200)) {
      // backMainPage 可能已通过 findPage→coinExchange 触发了新一轮任务流程并完成归位，
      // awaitingRewardClaim 已置 true，直接返回让外层循环继续
      setRunInfo(`mainPopupFn[${title}]: 完成后未检测到金币页，跳过等待`);
      return;
    }
    return;
  }
